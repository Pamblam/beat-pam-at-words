export class CanvasBoard {
  ...
  draw() {
    ...
    const canvas = this.createGridCanvas(this.width, this.height, cellSize, ({ col, row, rectDraw }) => {
      this.fillRectDraw(canvas, rectDraw, '#004400'); // uses precomputed rect; no re-calc
      console.log(col, row);
    });
  ...
  createGridCanvas(cols, rows, cellSize, onCellClick) {
    ...
    // Stash metrics for later
    canvas.__board = { cols, rows, cellSize, cssW, cssH, dpr };
  ...
  fillRectDraw(canvas, rectDraw, color) {
    const { dpr } = canvas.__board || { dpr: window.devicePixelRatio || 1 };
    const ctx = canvas.getContext('2d');
    ctx.save();
